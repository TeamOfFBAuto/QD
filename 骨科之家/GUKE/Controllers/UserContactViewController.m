...
- (void)addRedLable{
    ...
    if ([[[NSUserDefaults standardUserDefaults] objectForKey:@"isShowRed"] boolValue]) {
        ...
        if (redLable.frame.size.width == 0) {
            redLable.frame = CGRectMake(SCREEN_WIDTH/4 - 15, 7, 10, 10);
...
- (void)btnClick:(UIButton *)sender
{
    switch (sender.tag) {
        ...
        case CONTACT_BTN_TAG: {
            UserContactViewController *friendList = [[UserContactViewController alloc]init];
        ...
        case MOMENTS_BTN_TAG: {
            ...
            break;
            // 资料库
        case INFORMATION_BTN_TAG: {
            InformationViewController *information = [[InformationViewController alloc]init];
            [self.navigationController pushViewController:information animated:YES];
        }
            break;
            // 病历库MEDICAL_BTN_TAG
        case MEDICAL_BTN_TAG: {
            MedicalViewController *medical = [[MedicalViewController alloc]init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row < 3)
    {
        ...
        if ([model.contactType isEqualToString:ORDINARY_GROUP_CODE]) {
            ...
            cell.typeLable.text = @"";
            [cell.imgView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",IMAGE_BASE_URL,model.icon]] placeholderImage:[UIImage imageNamed:@"portrait_ico.png"]];
        }
