    ...
    if ([object isKindOfClass:[NSDictionary class]])///语音
    {
        ...
        if ([aDic objectForKey:@"fileurl"])
        {
            ...
            if([SNTools judgeFileSuffixVoice:url])///声音
            {
                ...
                self.files_label.text = [NSString _859ToUTF8:[aDic objectForKey:@"filename"]];
...
- (void)filesImageViewTap:(id)sender
{
    ...
    if ([_data_object isKindOfClass:[NSDictionary class]])///语音
    {
        ...
        {
            [self playLocalVoiceWithPath:[(NSMutableDictionary *)_data_object objectForKey:@"fileName"]];
        }
