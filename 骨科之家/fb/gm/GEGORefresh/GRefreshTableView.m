...
-(void)createHeaderView
{
    ...
    _refreshHeaderView = [[GEGORefreshTableHeaderView alloc]initWithFrame:CGRectMake(0.0f,0.0f -self.bounds.size.height, self.frame.size.width, self.bounds.size.height)];
...
-(void)showRefreshHeader:(BOOL)animated
{
    ...
    [_refreshHeaderView setState:GEGOOPullRefreshLoading];
    [_refreshHeaderView GEGORefreshScrollViewDidEndDragging:self];
}

#pragma mark - GEGORefreshTableDelegate
...
//根据刷新类型，是看是下拉还是上拉
-(void)beginToReloadData:(GEGORefreshPos)aRefreshPos
{
    //  should be calling your tableviews data source model to reload
    _reloading = YES;
    if (aRefreshPos ==  GEGORefreshHeader)
    {
        _isReloadData = YES;
...
- (void)finishReloadigData
{
    ...
    if (_refreshHeaderView) {
        [self.refreshHeaderView GEGORefreshScrollViewDataSourceDidFinishedLoading:self];
...
{
    if (_refreshHeaderView) {
        [_refreshHeaderView GEGORefreshScrollViewDidScroll:scrollView];
    }
}
- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate
{
    if (_refreshHeaderView)
    {
        [_refreshHeaderView GEGORefreshScrollViewDidEndDragging:scrollView];
    }
