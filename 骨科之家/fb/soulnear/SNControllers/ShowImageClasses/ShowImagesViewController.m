...
//    daohangView.image = FBCIRCLE_NAVIGATION_IMAGE;
    ...
    [button_back setImage:[UIImage imageNamed:@"guke_top_logo_arrow"] forState:UIControlStateNormal];
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo
{
    ...
    [SNTools showMBProgressWithText:@"保存图片成功" addToView:self.view];
}
...
- (void)viewDidLoad
{
    ...
    _myScrollView.showsHorizontalScrollIndicator = NO;
    _myScrollView.contentSize = CGSizeMake(DEVICE_WIDTH*self.allImagesUrlArray.count,0);
    [self.view addSubview:_myScrollView];
    
    _myScrollView.contentOffset = CGPointMake(DEVICE_WIDTH*_currentPage,0);

    for (int i = 0;i < self.allImagesUrlArray.count;i++)
    {
        UserArticleListAttachListModel * model = [self.allImagesUrlArray objectAtIndex:i];
        
        QBShowImagesScrollView * scrollView = [[QBShowImagesScrollView alloc] initWithFrame:CGRectMake(DEVICE_WIDTH*i,0,DEVICE_WIDTH,_myScrollView.frame.size.height) WithUrl:model.fileurl];
